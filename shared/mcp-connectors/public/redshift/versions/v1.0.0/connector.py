...
class RedshiftMCPServer(DestinationLoadMixin, BaseMCPConnector):
    ...
    # Which tables are user tables: base tables outside Redshift's catalogs
    # and the session temp schemas. discover_schema and list_namespaces both
    # filter by it, so the schema list and the table list never disagree.
    _USER_TABLES_WHERE = (
