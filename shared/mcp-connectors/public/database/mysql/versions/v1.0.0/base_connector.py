...
# =============================================================================
# JSON-SAFE ENCODING HELPERS (HTTP MODE SAFETY)
# =============================================================================
#
# In HTTP mode, FastAPI will attempt to JSON-encode the dict returned by
# handle_request(). Some DB drivers (notably mysql.connector) return `bytes` for
# BLOB/VARBINARY columns; without sanitization, FastAPI raises:
#   UnicodeDecodeError: 'utf-8' codec can't decode byte ...
#
# This helper converts non-JSON-safe values (bytes/bytearray) to a safe shape.
# We use a tagged base64 wrapper to preserve the original binary content.
#
def _json_safe(value: Any) -> Any:
    ...
    if isinstance(value, (bytes, bytearray)):
        b = bytes(value)
        return {"__binary_base64__": base64.b64encode(b).decode("ascii")}
    if isinstance(value, dict):
        return {str(k): _json_safe(v) for k, v in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_json_safe(v) for v in value]
    # Fall back to string for unknown types (keeps server resilient)
    return str(value)
...
# =============================================================================
# DestinationLoadMixin framework — grafted verbatim from the canonical
# shared/mcp-connectors/base_connector.py so this vendored per-connector snapshot
# is import-compatible with the connector's ``self.load_spec`` declaration.
# Canonical is the source of truth; keep this block in sync on the next regen.
# (This snapshot predates the framework, hence the graft rather than a full
# base swap, which would drag in unrelated canonical drift under a prod connector.)
# =============================================================================
    ...
    def handle_request(self, request: Dict) -> Dict:
        ...
        try:
            if method == 'tools/list':
                ...
                return _json_safe({"jsonrpc": "2.0", "id": request_id, "result": result})
                ...
                return _json_safe({"jsonrpc": "2.0", "id": request_id, "result": "pong"})
                ...
                return _json_safe({"jsonrpc": "2.0", "id": request_id, "error": {"code": -32601, "message": f"Method not found: {method}"}})
                
        except Exception as e:
            self.log(f"Request failed: {e}", level="error")
            import traceback
            self.log(traceback.format_exc(), level="debug")
            return _json_safe({"jsonrpc": "2.0", "id": request_id, "error": {"code": -32000, "message": str(e)}})
        finally:
            # Clean up trace_id
            clear_trace_id()
...
class ApiHandler:
    ...
    def export_resource(
        ...
        *,
        ...
        """
        logger.info(f"ApiHandler.export_resource: {resource} ({endpoint}), max_pages={max_pages}, max_records={max_records}")
        ...
        records, errors = self.pagination_handler.fetch_all_pages(
...
class StorageHandler:
    ...
    def export(
        ...
        *,
        ...
        Returns:
            ExportResult with file records, errors, and stats
        """
        logger.info(f"StorageHandler.export: bucket={bucket}, prefix={prefix}, max_files={max_files}")
        ...
        # Download files (required for validation)
        for obj in objects[:max_files]:
            ...
            try:
                ...
                records.append({
                    "key": obj.get('key') or obj.get('Key'),
                    "size": file_size,
                    "last_modified": obj.get('last_modified') or obj.get('LastModified'),
