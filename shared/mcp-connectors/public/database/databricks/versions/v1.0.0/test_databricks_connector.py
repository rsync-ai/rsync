...
def test_discover_lists_every_schema_with_the_configured_one_first():
    rows = [
        ...
        {"table_schema": "default", "table_name": "orders"},
        {"table_schema": "default", "table_name": "customers"},
        {"table_schema": "zz_audit", "table_name": "events"},
    ]
    s, conn = _db_fakes.make_connector(db, rows=rows)
    ...
    assert got == [("default", "customers"), ("default", "orders"),
                   ("sales", "invoices")], got
    ...
    assert "table_schema = ?" not in sql, sql
    assert "information_schema" in sql, sql


def test_discover_defaults_to_every_table_when_max_tables_is_unset():
    rows = [{"table_schema": "default", "table_name": f"t{i}"} for i in range(150)]
    s, _ = _db_fakes.make_connector(db, rows=rows)
