...
class BaseMCPConnector(ABC):
    ...
    def generate_destination_key(self, params: Dict, config: Dict, format_val: str, compression: str) -> str:
        ...
        # NOTE: second-resolution timestamps can collide when writing multiple
        # objects quickly (e.g. chunked batch exports). Include microseconds and
        # a short random suffix to avoid overwriting objects.
        import uuid
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S_%f')
        ...
        filename = f"data_{timestamp}_{suffix}.{ext}"
