...
// shared/sensitive_keys_golden.json, which backend-orchestrator's copy of this list
// and llm-service's SENSITIVE_KEYS are pinned to as well. The lists were kept in
// step by a comment; a key added to one service's log redaction and not the
// other's leaks that field from the second service's logs with no failing test.
