...
> [!NOTE]
> Multi-arch publishing applies to release tags cut after it landed. An older
> tag carries a single `linux/amd64` entry, and on an arm64 host it fails with
> `no matching manifest for linux/arm64/v8` — or, if a qemu handler happens to be
> registered, runs emulated and slowly. Check the tag you are about to install:
> ```bash
> docker manifest inspect ghcr.io/rsync-ai/api-gateway:<tag> | grep '"architecture"'
> ```
> If `arm64` is missing, use a newer tag or build natively from a checkout
...
- **ARM64 needs a multi-arch release tag** — tags cut before multi-arch publishing are `linux/amd64` only (see above)
